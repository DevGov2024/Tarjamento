...
from tkinter import Tk, Toplevel, Text, Scrollbar, Label, Button, filedialog, messagebox, RIGHT, Y, END, Toplevel,  Checkbutton, IntVar
...
def iniciar_interface():
    ...
    criar_botao("🔐 Tarjar PDF", tarjar_pdf_com_preview).pack(pady=5)
...
def tarjar_pdf_com_preview():
    ...
    # Etapa 2: Pré-visualizar com destaques (sem tarjar ainda)
    page = doc[0]
    texto_pagina = page.get_text()
    areas_para_tarjar = []
    ...
    for padrao in padroes_escolhidos.values():
        for ocorrencia in re.finditer(padrao, texto_pagina, re.IGNORECASE):
            texto_encontrado = ocorrencia.group()
            areas = page.search_for(texto_encontrado)
            areas_para_tarjar.extend(areas)

    # Desenhar destaque temporário em amarelo
    for area in areas_para_tarjar:
        page.draw_rect(area, color=(1, 1, 0), fill=(1, 1, 0), overlay=True)

    # Salvar imagem temporária para pré-visualização
    image_path = "preview_temp.png"
    pix = page.get_pixmap()
    pix.save(image_path)

    # Exibir janela de pré-visualização
    def aplicar_tarja():
        total_ocultados = 0
        has_redaction = hasattr(doc, "apply_redactions")
        for page in doc:
    ...
    janela = Toplevel()
    janela.title("Pré-visualização PDF com Destaques")
    imagem = Image.open(image_path)
    imagem_tk = ImageTk.PhotoImage(imagem)
    label = Label(janela, image=imagem_tk)
    label.image = imagem_tk
    ...
    Button(janela, text="Aplicar Tarja", command=aplicar_tarja, bg="black", fg="white").pack(pady=10)
    Button(janela, text="Cancelar", command=lambda: (doc.close(), janela.destroy())).pack(pady=5)


def selecionar_padroes2():
    padroes_escolhidos = {}
...
def criar_botao(texto, comando):
    return tk.Button(janela, text=texto, command=comando, width=30, bg="#4a90e2", fg="white", font=("Arial", 10, "bold"))
...
janela = tk.Tk()
janela.title("Tarjador Seletivo de PDF")
janela.mainloop()
